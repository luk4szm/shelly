...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const el = document.getElementById('chart-humidity');
    if (!el) return;
    ...
    const fetchData = async () => {
        ...
        try {
            const res = await fetch(`/weather/get-humidity?date=${encodeURIComponent(dateStr)}`, {cache: 'no-store'});
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
            return await res.json();
        } catch (e) {
            console.error('Błąd pobierania wilgotności:', e);
            return null;
        }
    };
    ...
    const transform = (raw) => {
        ...
        const data = raw
            ...
            .filter(Boolean)
            .sort((a, b) => a.x - b.x);
        return {series: [{name: 'Wilgotność', data}]};
    };
